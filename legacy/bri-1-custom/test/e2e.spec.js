"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
const types_1 = require("@baseline-protocol/types");
const chai_1 = require("chai");
const mocha_1 = require("mocha");
const shared_1 = require("./shared");
const utils_1 = require("./utils");
const aliceCorpName = 'Alice Corp';
const bobCorpName = 'Bob Corp';
const ropstenNetworkId = '66d44f30-9092-4182-a3c4-bc02736d6ae5';
const kovanNetworkId = '8d31bf48-df6b-4a71-9d7c-3cb291111e27';
const goerliNetworkId = '1b16996e-3595-4985-816c-043345d22f8c';
const networkId = process.env['NCHAIN_NETWORK_ID'] || ropstenNetworkId;
const setupUser = (identHost, firstname, lastname, email, password) => __awaiter(void 0, void 0, void 0, function* () {
    const user = (yield utils_1.createUser(identHost, firstname, lastname, email, password));
    const auth = yield utils_1.authenticateUser(identHost, email, password);
    const bearerToken = auth.token.token;
    chai_1.assert(bearerToken, `failed to authorize bearer token for user ${email}`);
    return [user, bearerToken];
});
mocha_1.describe('baseline', () => {
    let bearerTokens; // user API credentials
    let alice;
    let aliceApp;
    let bob;
    let bobApp;
    let workgroup;
    let workgroupToken;
    mocha_1.before(() => __awaiter(void 0, void 0, void 0, function* () {
        console.log("HEE DIT IS EEN TEST LEUK HE");
        yield utils_1.configureTestnet(5432, networkId);
        yield utils_1.configureTestnet(5433, networkId);
        const aliceUserToken = yield setupUser('localhost:8081', 'Alice', 'Baseline', `alice${new Date().getTime()}@baseline.local`, 'alicep455');
        alice = aliceUserToken[0];
        const bobUserToken = yield setupUser('localhost:8085', 'Bob', 'Baseline', `bob${new Date().getTime()}@baseline.local`, 'bobp455');
        bob = bobUserToken[0];
        bearerTokens = {};
        bearerTokens[alice['id']] = aliceUserToken[1];
        bearerTokens[bob['id']] = bobUserToken[1];
        // FIXME -- vend unique keypair for each participant
        const natsPrivateKey = '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----';
        const natsPublicKey = '-----BEGIN PUBLIC KEY-----\\n-----END PUBLIC KEY-----';
        aliceApp = yield utils_1.baselineAppFactory(aliceCorpName, bearerTokens[alice['id']], false, 'localhost:8081', 'nats://localhost:4222', natsPrivateKey, natsPublicKey, 'localhost:8080', networkId, 'localhost:8082', 'localhost:8084', 'localhost:4001/jsonrpc', 'http', null, 'baseline workgroup', null, 'corn domain lonely owner media grape hard rough arena knock uncover goddess cinnamon wing actress spring dizzy skill alter pistol funny bind rapid soap');
        bobApp = yield utils_1.baselineAppFactory(bobCorpName, bearerTokens[bob['id']], true, 'localhost:8085', 'nats://localhost:4224', natsPrivateKey, natsPublicKey, 'localhost:8086', networkId, 'localhost:8083', 'localhost:8087', 'localhost:4002/jsonrpc', 'http', null, 'baseline workgroup', null, 'forest step weird object extend boat ball unit canoe pull render monkey drink monitor behind supply brush frown alone rural minute level host clock');
        yield bobApp.init();
        yield aliceApp.init();
    }));
    after('close connections', () => __awaiter(void 0, void 0, void 0, function* () {
        yield bobApp.disconnect();
        yield aliceApp.disconnect();
    }));
    mocha_1.describe('workgroup', () => {
        mocha_1.describe('creation', () => {
            mocha_1.before(() => __awaiter(void 0, void 0, void 0, function* () {
                yield bobApp.requireWorkgroup();
                workgroup = bobApp.getWorkgroup();
                workgroupToken = bobApp.getWorkgroupToken();
            }));
            it('should create the workgroup in the local registry', () => __awaiter(void 0, void 0, void 0, function* () {
                chai_1.assert(workgroup, 'workgroup should not be null');
                chai_1.assert(workgroup.id, 'workgroup id should not be null');
            }));
            it('should authorize a bearer token for the workgroup', () => __awaiter(void 0, void 0, void 0, function* () {
                chai_1.assert(workgroupToken, 'workgroup token should not be null');
            }));
            it('should deploy the ERC1820 registry contract for the workgroup', () => __awaiter(void 0, void 0, void 0, function* () {
                const erc1820RegistryContract = yield bobApp.requireWorkgroupContract('erc1820-registry');
                chai_1.assert(erc1820RegistryContract, 'workgroup ERC1820 registry contract should not be null');
            }));
            it('should deploy the ERC1820 organization registry contract for the workgroup', () => __awaiter(void 0, void 0, void 0, function* () {
                const orgRegistryContract = yield bobApp.requireWorkgroupContract('organization-registry');
                chai_1.assert(orgRegistryContract, 'workgroup organization registry contract should not be null');
            }));
        });
        mocha_1.describe('participants', () => {
            mocha_1.before(() => __awaiter(void 0, void 0, void 0, function* () {
                // sanity check
                chai_1.assert(alice && bob, 'an administrative user should have been created for each workgroup counterparty');
                chai_1.assert(Object.keys(bearerTokens).length === 2, 'a bearer token should have been authorized for each administrative user');
                chai_1.assert(aliceApp, 'an instance should have been initialized for Alice Corp');
                chai_1.assert(bobApp, 'an instance should have been initialized for Bob Corp');
                chai_1.assert(workgroup, 'workgroup should not be null');
                chai_1.assert(workgroupToken, 'workgroup token should not be null');
            }));
            mocha_1.describe('workgroup initiator', () => {
                mocha_1.describe(`initial workgroup organization: "${bobCorpName}"`, shared_1.shouldBehaveLikeAnInitialWorkgroupOrganization(() => bobApp));
                mocha_1.describe(`workgroup organization: "${bobCorpName}"`, shared_1.shouldBehaveLikeAWorkgroupOrganization(() => bobApp));
            });
            mocha_1.describe('inviting participants to the workgroup', function () {
                let inviteToken;
                mocha_1.before(() => __awaiter(this, void 0, void 0, function* () {
                    yield bobApp.inviteWorkgroupParticipant(alice.email);
                    inviteToken = yield utils_1.scrapeInvitationToken('bob-ident-consumer'); // if configured, ident would have sent an email to Alice
                }));
                it('should have created an invite for alice', () => __awaiter(this, void 0, void 0, function* () {
                    chai_1.assert(inviteToken, 'invite token should not be null');
                }));
                mocha_1.describe('alice', function () {
                    mocha_1.before(() => __awaiter(this, void 0, void 0, function* () {
                        yield bobApp.requireWorkgroupContract('erc1820-registry');
                        yield bobApp.requireWorkgroupContract('organization-registry');
                        yield aliceApp.acceptWorkgroupInvite(inviteToken, bobApp.getWorkgroupContracts());
                    }));
                    mocha_1.describe(`invited workgroup organization: "${aliceCorpName}"`, shared_1.shouldBehaveLikeAnInvitedWorkgroupOrganization(() => aliceApp));
                    mocha_1.describe(`workgroup organization: "${aliceCorpName}"`, shared_1.shouldBehaveLikeAWorkgroupOrganization(() => aliceApp));
                    mocha_1.describe(`workgroup counterparty: "${aliceCorpName}"`, shared_1.shouldBehaveLikeAWorkgroupCounterpartyOrganization(() => aliceApp));
                });
                mocha_1.describe('counterparties post-onboarding', function () {
                    mocha_1.describe(bobCorpName, shared_1.shouldBehaveLikeAWorkgroupCounterpartyOrganization(() => bobApp));
                });
            });
            mocha_1.describe('workflow', () => {
                mocha_1.describe('workstep', () => {
                    mocha_1.before(() => __awaiter(void 0, void 0, void 0, function* () {
                        const recipient = yield aliceApp.resolveOrganizationAddress();
                        yield bobApp.sendProtocolMessage(recipient, types_1.Opcode.Baseline, {
                            id: 'uuidv4()',
                            name: 'hello world',
                            url: 'proto://deep/link/to/doc',
                            rfp_id: null,
                        });
                    }));
                    it('should increment protocol message tx count for the sender', () => __awaiter(void 0, void 0, void 0, function* () {
                        chai_1.assert(bobApp.getProtocolMessagesTx() === 2, 'protocol messages tx should equal 2');
                    }));
                    it('should increment protocol message rx count for the recipient', () => __awaiter(void 0, void 0, void 0, function* () {
                        yield utils_1.promisedTimeout(50);
                        chai_1.assert(aliceApp.getProtocolMessagesRx() === 2, 'protocol messages rx should equal 2');
                    }));
                });
            });
        });
    });
});
//# sourceMappingURL=e2e.spec.js.map