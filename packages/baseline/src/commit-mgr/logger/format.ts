...
// Formats
export const defaultConsoleFormat = winston.format.printf(({ timestamp, level, service, message, ...args }) => {
...
export const reqHttpConsoleFormat = winston.format.printf(({ timestamp, level, message, meta }) => {
  const timestampFormated = timestamp.slice(0, 19).replace("T", " ");
  level = "http";
...
export const defaultFileFormat = winston.format.printf(({ timestamp, level, message, ...args }) => {
  const timestampFormated = timestamp.slice(0, 19).replace("T", " ");
...
// TODO: handle batched requests
export const reqHttpFileFormat = winston.format.printf(({ timestamp, level, message, meta }) => {
  const timestampFormated = timestamp.slice(0, 19).replace("T", " ");
  level = "http";
  const rpcMethod = meta.req.body ? meta.req.body.method : "";
  ...
  return (
    `[${timestampFormated}] [${level.toUpperCase()}] [${service}] [${os.hostname()}]: ${meta.req.method} ` +
