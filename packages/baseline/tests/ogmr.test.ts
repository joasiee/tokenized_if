...
import { orgregistry, OrganizationsService } from "../src/organization-mgr";
...
const getOrganization = function (name: string, address: string, keys: string[]) {
  return new Organization().setName(name).setAddress(address).setMsgkey(keys[0]).setMsgurl(keys[1]).setZkpkey(keys[2]);
};
