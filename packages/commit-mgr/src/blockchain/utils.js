...
export const get_ws_provider = () => {
  if (!ws_provider) {
    try {
      ...
      logger.info(
        `Established websocket connection: ${process.env.CMGR_ETH_CLIENT_WS}`
      );
    } catch (err) {
      logger.error(
        `[WEBSOCKET] Cannot establish connection: ${process.env.CMGR_ETH_CLIENT_WS}`
      );
    }
  }
  return ws_provider;
